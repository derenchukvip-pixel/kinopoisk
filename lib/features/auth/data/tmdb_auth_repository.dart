...
class TMDBAuthRepository implements AuthRepository {
  ...
  Future<AuthUser?> login(String username, String password) async {
    // 1. Get request token
    final tokenResp = await http.get(Uri.parse('$_baseUrl/authentication/token/new?api_key=$apiKey'));
    if (tokenResp.statusCode != 200) throw Exception('Failed to get request token');
    final requestToken = jsonDecode(tokenResp.body)['request_token'];

    // 2. Validate token with login
    final validateResp = await http.post(
      Uri.parse('$_baseUrl/authentication/token/validate_with_login?api_key=$apiKey'),
  ...
  Future<void> logout() async {
    ...
    await http.delete(
      Uri.parse('$_baseUrl/authentication/session?api_key=$apiKey'),
      headers: {'Content-Type': 'application/json'},
